...
1. 使用仓库准备工具获取最高不超过 1080p 的完整视频、48 kHz 分析音频、元数据、全片抽帧和联系表，输出到 `.work/runs/<videoId>/`。
2. 核对视频时长、画面可读性和音频完整性。字幕仅用于搜索和时间定位。
3. 先查看全片联系表建立章节结构，再逐段检查原画幅帧，精看插件、参数、路由和自动化出现的关键时间点。
...
- 只有字幕、缩略图或低清不可读画面时，该条保持未完成，不能创建占位记录。
